...
func QueryGoodsPage(order string, currentPage int, pageSize int) (map[string]interface{}, error) {
	GoodsList, err := model.QueryGoodsPage(order, currentPage, pageSize)
	if err != nil {
		return nil, err
	}
	count, err := model.QueryGoodsCount(order)
	if err != nil {
		return nil, err
	}
